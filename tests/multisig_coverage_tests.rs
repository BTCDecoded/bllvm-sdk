...
fn test_multisig_verify_edge_cases() {
    ...
    let signatures: Vec<_> = (0..10).map(|i| {
...
fn test_multisig_collect_valid_signatures_edge_cases() {
    ...
    let signatures: Vec<_> = keypairs.iter().map(|kp| {
        sign_message(&kp.secret_key, message).unwrap()
    }).collect();
...
fn test_multisig_threshold_properties() {
    let keypairs: Vec<_> = (0..7).map(|_| GovernanceKeypair::generate().unwrap()).collect();
    let public_keys: Vec<_> = keypairs.iter().map(|kp| kp.public_key()).collect();
