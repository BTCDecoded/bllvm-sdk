...
use developer_sdk::governance::{GovernanceKeypair, PublicKey, GovernanceError};
...
fn test_governance_error_display() {
    ...
    let error = GovernanceError::InvalidThreshold { threshold: 2, total: 1 };
    let display_str = format!("{}", error);
...
fn test_governance_error_variants() {
    ...
    let error7 = GovernanceError::InvalidThreshold { threshold: 1, total: 2 };
